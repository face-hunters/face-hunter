...
class FlaskApi(object):
    ...
    def __init__(self,
      ...
      self.recognizer_model = ApproximateKNearestNeighbors(distance_threshold=distance_threshold, index_path=index_path)
      self.face_recognition = FaceRecognition(thumbnail_list, thumbnails_path, img_width, distance_threshold, encoder_name, labels_path, embeddings_path)
      self.graph = Graph(storage_type, memory_path, virtuoso_url, virtuoso_graph, virtuoso_username, virtuoso_password, dbpedia_csv, wikidata_csv)
    ...
    def recognize_youtube_video(self, identifier, by='frame', frame_threshold=7):
      ...
      if self.graph.video_exists(identifier):
        result = [scene[1:] for scene in self.graph.get_scenes_from_video(identifier)]  # [scene, entity, start, end]
        return result

      path_to_video = download_youtube_video(f'https://www.youtube.com/watch?v={identifier}', tempfile.gettempdir())

      recognized_entities, frame_wise_entities, timestamps = self.face_recognition.recognize_video(path_to_video, self.recognizer_model, by)

      graph.insert_video(identifier, os.path.split(path_to_video)[1])
      scenes = extract_scenes(frame_wise_entities, timestamps, frame_threshold)
      for scene in scenes:
        result.append([scene.names[0].tolist(), str(scene.start[0]).split('.')[0], str(scene.end[0]).split('.')[0]])  # accurate to second
