...
def tune_distance_threshold(video_path='data/datasets/youtube_faces', sample_per_person=5, model='Dlib'):  # TODO: change path
  """ tune distance threshold

  Parameters:
    video_path: youtube faces data set path
    sample_per_person: how many frames to compare in the video data set
    model: the face recognition model to tune
  ...
  if not os.path.exists(thumbnail_path):
    print('1/7 cleaning and matching thumbnails...')
    ...
    video_name_df = video_name_df.merge(check_duplicate, how='inner', left_on='entities', right_on='norm_name').drop(['norm_name'], axis=1)
    ...
    print(f'the distance threshold tuning is running on {len(tmp.name.unique())} celebrities')  # TODO : test
    print('2/7 downloading thumbnails...')
  ...
  print('3/7 creating thumbnails embeddings...')
  fr = FaceRecognition(thumbnails_path=thumbnails_path, encoder_name=model, labels_path=labels_path, embeddings_path=embeddings_path)

  # 3 create train data set
  print('4/7 creating train data set...')
  ...
  for i, row in dataset.iterrows(): # for every thumbnail
    ...
    sample_per_thumbnail = match_dataset[match_dataset['labels'] != label].sample(n=sample_per_person, random_state=42)
  ...
  print('5/7 creating embeddings of frames...')
  ...
  print(f'{remove_number} frames contains more than 1 face')
  ...
  print('6/7 calculating distances...')
  cosine_distances = []
  euclidean_distances = []
  ...
  for i, row in dataset.iterrows():
    ...
    cosine_distance = 1- np.matmul(embedding_thumbnail, embedding_frame) / (np.linalg.norm(embedding_thumbnail) * np.linalg.norm(embedding_frame))
    cosine_distances.append(cosine_distance)

    euclidean_distance = np.linalg.norm(embedding_thumbnail - embedding_frame)
    euclidean_distances.append(euclidean_distance)

    euclidean_l2_distance = np.linalg.norm(embedding_thumbnail/np.linalg.norm(embedding_thumbnail) - embedding_frame/np.linalg.norm(embedding_frame))
    euclidean_l2_distances.append(euclidean_l2_distance)

  dataset['cosine_distances'] = cosine_distances # { ix, labels, file, entities, identical, cosine_distances, euclidean_distances, euclidean_l2_distances }
  ...
  print('7/7 running decision tree and evaluating...')
